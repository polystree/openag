      ...
      try {
        switch (message.action) {
          ...
          case "clearLogs":
            ...
            break;
          case "toggleAutoRun": {
            const status = AutoRunPatcher.getStatus();
            const res = status.isPatched ? AutoRunPatcher.revert() : AutoRunPatcher.apply();
            this.log(`[AutoRun Fix] ${res.message}`);
            if (res.success) {
...
.term{background:var(--card);border:1px solid var(--border);border-radius:6px;height:180px;max-height:25vh;overflow-y:auto;padding:6px;font-family:var(--mono);font-size:9.5px;display:flex;flex-direction:column;gap:3px;min-width:0;user-select:text;-webkit-user-select:text;cursor:text}
.term *{user-select:text;-webkit-user-select:text}.log-line{display:flex;gap:4px;line-height:1.3;word-break:break-all;min-width:0;user-select:text;-webkit-user-select:text}.log-tag{font-size:8px;font-weight:700;padding:0 2px;border-radius:2px;flex-shrink:0;background:rgba(255,255,255,.1)}.empty{padding:12px;text-align:center;color:var(--dim);font-size:10.5px}
</style></head><body>
<div class="row" style="padding-bottom:6px;border-bottom:1px solid var(--border);"><div style="display:flex;align-items:center;gap:5px;"><span class="badge">OPENAG</span><button id="b-autorun" class="btn btn-sec" style="font-size:9px;padding:1px 5px;border-radius:3px;" title="Terminal Auto-Run Fix status">Auto-Run</button></div><label class="switch" title="Toggle OpenAG"><input type="checkbox" id="cfg-enabled" checked /><span class="slider"></span></label></div>
...
document.getElementById('b-add').onclick=()=>send('addAccount');document.getElementById('b-refresh').onclick=()=>send('refreshQuotas');document.getElementById('b-autorun').onclick=()=>send('toggleAutoRun');document.getElementById('cfg-enabled').onchange=(e)=>send('updateConfig',{enabled:e.target.checked});
...
function render(){if(state.config)document.getElementById('cfg-enabled').checked=state.config.enabled??true;const abtn=document.getElementById('b-autorun');if(abtn&&state.patcher){if(state.patcher.isPatched){abtn.textContent='Auto-Run ✓';abtn.style.color='#10b981';abtn.style.borderColor='rgba(16,185,129,.4)';abtn.title='Auto-Run Fix is active (click to revert)';}else{abtn.textContent='Auto-Run Fix';abtn.style.color='var(--dim)';abtn.style.borderColor='var(--border)';abtn.title='Auto-Run Fix is available (click to apply)';}}renderAccounts();renderLogs();}
